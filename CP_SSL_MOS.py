...
class MosPredictor(nn.Module):
    def __init__(self, ssl_model, ssl_out_dim):
        ...
        self.dense1 = nn.Linear(self.ssl_features+1, 1)
        self.layernorm = nn.LayerNorm(self.ssl_features+1)
    ...
    def forward(self, wav, pitchs):
        ...
        if t1 == t2:
            ...
            pitchs = pitchs[:, :t1-t2, :]
            x_concat_pitch = torch.cat([x,pitchs],dim = -1)
        x_concat_pitch = torch.mean(x_concat_pitch, 1).float()
        x_concat_pitch = self.layernorm(x_concat_pitch)
        x_concat_pitch = self.dense1(x_concat_pitch)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        index_values = [10 * (12 * np.log2(p / 440) % 12) if p != 0 else -1 for p in pitch ]
        int_index = [int(np.floor(value)) for value in index_values]
        pitch_index = np.array(int_index)
        return wav, score, wavname, pitch_index
    ...
    def collate_fn(self, batch):  
        wavs, scores, wavnames, pitch_index= zip(*batch)
        wavs = list(wavs)
        ...
        b = len(pitch_index) 
        max_len2 = max([len(arr) for arr in pitch_index]) 
        output_pitchs = torch.full((b, max_len2), -1)
        for i, arr in enumerate(pitch_index):
            output_pitchs[i, :len(arr)] = torch.tensor(arr)
        output_wavs = torch.stack(output_wavs, dim=0)
        scores  = torch.stack([torch.tensor(x) for x in list(scores)], dim=0)
        return output_wavs, scores, wavnames, output_pitchs
...
if __name__ == '__main__':
    ...
    for epoch in range(1,1001):
        ...
        for i, data in enumerate(trainloader, 0):
            inputs, labels, filenames, output_pitchs = data
            inputs = inputs.to(device)
            labels = labels.to(device)
            output_pitchs = output_pitchs.to(device)
            optimizer.zero_grad()
            outputs = net(inputs, output_pitchs)
